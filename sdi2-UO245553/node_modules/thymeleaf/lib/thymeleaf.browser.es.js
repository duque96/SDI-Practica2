...
const TemplateName = new Rule('TemplateName', /[\w-\._]+/);

const FragmentName = new Rule('FragmentName', /[\w-\._]+/);

// TODO: We're not doing anything with these yet
const FragmentParameters = new Rule('FragmentParameters', new OptionalExpression(/\(.+\)/));
 ...
 * @author Emanuel Rabina
 */
var FragmentExpression = new Rule('FragmentExpression', new SequenceExpression(/~{/, 'TemplateName', /::/, 'FragmentName', 'FragmentParameters', /}/), (_ref) => {
	var _ref2 = slicedToArray(_ref, 5);

	let templateName = _ref2[1],
	    fragmentName = _ref2[3],
	    parameters = _ref2[4];
	return context => {
 ...
 * @author Emanuel Rabina
 */
var Identifier = new Rule('Identifier', /[a-zA-Z_][\w\.]*/);
 ...
 * @author Emanuel Rabina
 */
var VariableExpression = new Rule('VariableExpression', new SequenceExpression(/\${/, Identifier.name, /\}/), (_ref) => {
 ...
 * @author Emanuel Rabina
 */
var Iteration = new Rule('Iteration', new SequenceExpression(Identifier.name, /:/, VariableExpression.name), (_ref) => {
  var _ref2 = slicedToArray(_ref, 3);

  let localValueName = _ref2[0],
      collectionExpressionAction = _ref2[2];
  return context => {
    return {
      localValueName,
 ...
 * @author Emanuel Rabina
 */
class RegularExpressionMatchProcessor {
...
const Url = new Rule('Url', /.+/);

const UrlParameters = new Rule('UrlParameters', /\((.+)\)/);
 ...
 * @author Emanuel Rabina
 */
var LinkExpression = new Rule('LinkExpression', new RegularExpressionMatchProcessor(/^@\{(.+?)(\(.+\))?\}$/, [Url.name, UrlParameters.name]), (_ref) => {
	var _ref2 = slicedToArray(_ref, 3);
		...
		if (parameters) {
			...
			while (true) {
				...
				if (urlTemplate) {
					...
					let paramEntry = remove(paramsList, (_ref3) => {
 ...
 * @author Emanuel Rabina
 */
var BooleanLiteral = new Rule('BooleanLiteral', /(true|false)/, result => () => {
  return result === 'true';
});
 ...
 * @author Emanuel Rabina
 */
var NullLiteral = new Rule('NullLiteral', /null/, () => () => {
 ...
 * @author Emanuel Rabina
 */
var NumberLiteral = new Rule('NumberLiteral', /\d+(\.\d+)?/, result => () => {
 ...
 * @author Emanuel Rabina
 */
var StringLiteral = new Rule('StringLiteral', /'.*?'/, result => () => {
 ...
 * @author Emanuel Rabina
 */
var TokenLiteral = new Rule('TokenLiteral', /[^: $\{\}]+/, result => () => {
  return result;
});
 ...
 */
var Literal = new Rule('Literal', new OrderedChoiceExpression(StringLiteral.name, NumberLiteral.name, BooleanLiteral.name, NullLiteral.name, TokenLiteral.name));
 ...
 * @author Emanuel Rabina
 */
var Operand = new Rule('Operand', new OrderedChoiceExpression(VariableExpression.name, Literal.name));
...
const Comparator = new Rule('Comparator', new OrderedChoiceExpression(/===?/));
 ...
 * @author Emanuel Rabina
 */
var LogicalExpression = new Rule('LogicalExpression', new SequenceExpression(Operand.name, Comparator.name, Operand.name), (_ref) => {
  var _ref2 = slicedToArray(_ref, 3);

  let leftOperand = _ref2[0],
      comparator = _ref2[1],
      rightOperand = _ref2[2];
  return context => {
 ...
 * @author Emanuel Rabina
 */
var Condition = new Rule('Condition', new OrderedChoiceExpression(LogicalExpression.name, Operand.name));
 ...
 * @author Emanuel Rabina
 */
var IfThenCondition = new Rule('IfThenCondition', new SequenceExpression(Condition.name, /\?/, Operand.name), (_ref) => {
  var _ref2 = slicedToArray(_ref, 3);

  let condition = _ref2[0],
      truthyExpression = _ref2[2];
  return context => {
 ...
 * @author Emanuel Rabina
 */
var IfThenElseCondition = new Rule('IfThenElseCondition', new SequenceExpression(Condition.name, /\?/, Operand.name, /:/, Operand.name), (_ref) => {
  var _ref2 = slicedToArray(_ref, 5);

  let condition = _ref2[0],
      truthyExpression = _ref2[2],
      falseyExpression = _ref2[4];
  return context => {
 ...
 * @author Emanuel Rabina
 */
var ThymeleafExpression = new Rule('ThymeleafExpression', new OrderedChoiceExpression(new AllInputExpression(VariableExpression.name), new AllInputExpression(LinkExpression.name), new AllInputExpression(FragmentExpression.name), new AllInputExpression(Iteration.name), new AllInputExpression(IfThenCondition.name), new AllInputExpression(Literal.name), new AllInputExpression(LogicalExpression.name), new AllInputExpression(IfThenCondition.name), new AllInputExpression(IfThenElseCondition.name), new AllInputExpression(Nothing.name)));
 ...
 * @author Emanuel Rabina
 */
var ThymeleafExpressionLanguage = new Grammar('Thymeleaf Expression Language', ThymeleafExpression,

// Ordered as at https://www.thymeleaf.org/doc/tutorials/3.0/usingthymeleaf.html#standard-expression-syntax

// Simple expressions
VariableExpression, LinkExpression, Url, UrlParameters, FragmentExpression, TemplateName, FragmentName, FragmentParameters,

// Complex expressions
Iteration,

// Literals
Literal, StringLiteral, NumberLiteral, BooleanLiteral, NullLiteral, TokenLiteral,
...
LogicalExpression, Comparator,

// Conditional operators
IfThenCondition, IfThenElseCondition, Condition,
...
var dumbQuerySelector_1 = dumbQuerySelector.$;
			...
			return regenerator.wrap(function _callee$(_context) {
				while (1) switch (_context.prev = _context.next) {
					...
					case 10:
						...
						fragment = dumbQuerySelector_1(`[${dialectPrefix}\\:${fragmentProcessorName}^="${fragmentName}"]`, template) || dumbQuerySelector_1(`[data-${dialectPrefix}-${fragmentProcessorName}^="${fragmentName}"`, template);
...
class TemplateEngine {
  ...
  */
	processFile(filePath) {
		return Promise.reject(new Error('Cannot use fs.readFile inside a browser'));
	}
...
//# sourceMappingURL=thymeleaf.browser.es.js.map
